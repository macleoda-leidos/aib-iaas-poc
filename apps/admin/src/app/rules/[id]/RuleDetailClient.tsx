...
export default function RuleDetailClient({ rule }: { rule: RuleDefinition }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      {/* Back link */}
      <Link href="/rules" className="text-blue-700 hover:underline text-sm mb-6 inline-block">
        &larr; Back to Rules
      </Link>
      ...
      <div className="mb-8">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="inline-block px-2 py-0.5 text-xs font-mono font-medium rounded bg-blue-100 text-blue-800 border border-blue-300">
            v{rule.version}
          </span>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div>
            ...
            <p className="text-sm font-semibold text-gray-900 mt-1">{formatDate(rule.lastUpdated)}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Updated By</p>
            <p className="text-sm font-semibold text-gray-900 mt-1">{rule.updatedBy}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Priority</p>
            <p className="text-sm font-semibold text-gray-900 mt-1">{rule.priority}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Target Product</p>
            <p className="text-sm font-semibold text-gray-900 mt-1">{rule.product}</p>
          </div>
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Engine Version</p>
            <p className="text-sm font-semibold text-gray-900 mt-1">v2.3</p>
          </div>
          <div>
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Test Coverage</p>
            <p className="text-sm font-semibold text-gray-900 mt-1">{rule.testResults.coverage}%</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6 border-l-4 border-blue-500">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Rule Conditions (IF)</h2>
        <div className="space-y-3">
          {rule.conditions.map((condition, index) => (
            <div key={index}>
              {index > 0 && (
                <div className="flex items-center my-2">
                  <span className="text-xs font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded">AND</span>
                </div>
              )}
              <div className="bg-gray-50 border border-gray-200 rounded p-3">
                <code className="text-sm font-mono text-gray-800">
                  IF <span className="text-blue-700">{condition.field}</span>{' '}
                  <span className="text-red-600">{condition.operator}</span>{' '}
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6 border-l-4 border-green-500">
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Actions (THEN)</h2>
        <div className="space-y-3">
          {rule.actions.map((action, index) => (
            <div key={index} className="bg-green-50 border border-green-200 rounded p-3">
              ...
              <p className="text-xs text-green-700 mt-1">{action.displayText}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
        <h2 className="text-lg font-semibold text-gray-900 mb-2">Test This Rule</h2>
        <p className="text-sm text-gray-500 mb-4">Enter sample applicant data to see if this rule would trigger.</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Monthly Disposable Income (£)</label>
            <input
              type="number"
          ...
          <div className="flex items-center gap-2 pt-5">
            ...
            />
            <label htmlFor="hasExistingCase" className="text-sm text-gray-700">Has active case in another system</label>
          </div>
          <div className="flex items-center gap-2 pt-5">
            <input
              type="checkbox"
              ...
              className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
            />
            <label htmlFor="hasMoratorium" className="text-sm text-gray-700">Active Moratorium</label>
          </div>
        </div>
          ...
          <div className="mt-6">
            {testResult.matches ? (
              <div className="bg-green-50 border border-green-300 rounded p-4 mb-4">
                <p className="text-green-800 font-semibold">✓ RULE MATCHES — Would recommend: {rule.product}</p>
              </div>
            ) : (
              <div className="bg-gray-100 border border-gray-300 rounded p-4 mb-4">
                <p className="text-gray-700 font-semibold">✗ RULE DOES NOT MATCH</p>
              </div>
            )}
            ...
            <tbody>
              {rule.changeHistory.map((entry, index) => (
                <tr key={index} className={`border-b border-gray-100 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                  <td className="px-4 py-3 font-mono text-gray-600">v{entry.version}</td>
                  <td className="px-4 py-3 text-gray-600">{formatDate(entry.date)}</td>
                  <td className="px-4 py-3 text-gray-700">{entry.author}</td>
