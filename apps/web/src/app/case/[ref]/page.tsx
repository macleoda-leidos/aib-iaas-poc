...
const CASES: Record<string, any> = {
  ...
  'IAAS-2026-00011': { ref: 'IAAS-2026-00011', debtor: { name: 'Brenda Campbell', dob: '1975-04-18', nino: 'CD987654B', address: '8 Castle Street, Glasgow, G2 1AB', phone: '07700900011', email: 'b.campbell@example.com', employment: 'Unemployed', maritalStatus: 'Single', dependants: 0 }, status: 'under_review', product: 'MAP', assignedTo: 'James Wilson', totalDebt: 9200, submittedAt: '28 Jun 2026', creditScore: 340, creditResult: 'FAIL',
    ...
    notes: [{ author: 'System', date: '28 Jun 15:20', content: 'Application submitted', type: 'system' }, { author: 'System', date: '28 Jun 15:21', content: 'Credit check — score 340 (FAIL). 2 defaults found.', type: 'system' }, { author: 'James Wilson', date: '29 Jun 09:15', content: 'Reviewing application. Credit score below threshold but MAP eligibility likely met. Checking assets.', type: 'staff' }],
...
// Default case for unknown refs
const DEFAULT_CASE = { ref: 'Unknown', debtor: { name: 'Unknown', dob: '', nino: '', address: '', phone: '', email: '', employment: '', maritalStatus: '', dependants: 0 }, status: 'draft', product: '—', assignedTo: '—', totalDebt: 0, submittedAt: '—', creditScore: 0, creditResult: '—', debts: [], documents: [], notes: [], transactions: [] };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-0 min-w-max">
          {tabs.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2.5 text-sm font-medium border-b-2 whitespace-nowrap ${activeTab === tab ? 'border-blue-600 text-blue-700' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white border border-gray-200 rounded p-4">
            ...
            <dl className="space-y-2 text-sm">
              <div className="flex justify-between"><dt className="text-gray-500">Name</dt><dd className="font-medium">{caseData.debtor.name}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-500">Date of Birth</dt><dd>{caseData.debtor.dob}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-500">NI Number</dt><dd className="font-mono">{caseData.debtor.nino}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-500">Address</dt><dd className="text-right max-w-[200px]">{caseData.debtor.address}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-500">Phone</dt><dd>{caseData.debtor.phone}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-500">Email</dt><dd>{caseData.debtor.email}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-500">Employment</dt><dd>{caseData.debtor.employment}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-500">Marital Status</dt><dd>{caseData.debtor.maritalStatus}</dd></div>
              <div className="flex justify-between"><dt className="text-gray-500">Dependants</dt><dd>{caseData.debtor.dependants}</dd></div>
            </dl>
          </div>
          <div className="space-y-4">
            <div className="bg-white border border-gray-200 rounded p-4">
              ...
              <dl className="space-y-2 text-sm">
                ...
                <div className="flex justify-between"><dt className="text-gray-500">Recommended Product</dt><dd className="font-bold">{caseData.product}</dd></div>
                ...
                <div className="flex justify-between"><dt className="text-gray-500">Documents</dt><dd>{caseData.documents.length}</dd></div>
              </dl>
            </div>
      ...
      {activeTab === 'debts' && (
        <div className="bg-white border border-gray-200 rounded overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50"><tr><th className="text-left p-3">Creditor</th><th className="text-left p-3">Type</th><th className="text-right p-3">Amount</th><th className="text-right p-3">Monthly</th></tr></thead>
            <tbody>
              {caseData.debts.map((d: any, i: number) => (
                <tr key={i} className="border-b"><td className="p-3 font-medium">{d.creditor}</td><td className="p-3">{d.type}</td><td className="p-3 text-right font-bold">£{d.amount.toLocaleString()}</td><td className="p-3 text-right">£{d.monthly}</td></tr>
              ))}
              <tr className="bg-gray-50 font-bold"><td className="p-3">TOTAL</td><td></td><td className="p-3 text-right">£{caseData.totalDebt.toLocaleString()}</td><td className="p-3 text-right">£{caseData.debts.reduce((s: number, d: any) => s + d.monthly, 0)}</td></tr>
            </tbody>
          </table>
        </div>
      )}
      ...
      {activeTab === 'documents' && (
        <div className="bg-white border border-gray-200 rounded overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50"><tr><th className="text-left p-3">File</th><th className="text-left p-3">Category</th><th className="text-left p-3">Size</th><th className="text-left p-3">Uploaded</th><th className="text-left p-3">Scan</th></tr></thead>
            <tbody>
              {caseData.documents.map((d: any, i: number) => (
                <tr key={i} className="border-b"><td className="p-3 font-medium">📄 {d.name}</td><td className="p-3">{d.category}</td><td className="p-3">{d.size}</td><td className="p-3">{d.uploaded}</td><td className="p-3"><span className="bg-green-100 text-green-800 px-2 py-0.5 rounded text-xs font-bold">{d.status}</span></td></tr>
              ))}
            </tbody>
          </table>
        </div>
      ...
      {activeTab === 'transactions' && (
        <div className="bg-white border border-gray-200 rounded overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50"><tr><th className="text-left p-3">Date</th><th className="text-left p-3">Action</th><th className="text-left p-3">Actor</th><th className="text-left p-3">Status</th></tr></thead>
            <tbody>
              {caseData.transactions.map((t: any, i: number) => (
                <tr key={i} className="border-b"><td className="p-3 text-xs">{t.date}</td><td className="p-3 font-medium">{t.action}</td><td className="p-3">{t.actor}</td><td className="p-3"><span className={`text-xs font-bold ${t.status === 'ok' ? 'text-green-700' : 'text-amber-700'}`}>{t.status === 'ok' ? '✓' : '⚠'}</span></td></tr>
              ))}
            </tbody>
          </table>
        </div>
