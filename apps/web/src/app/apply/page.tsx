...
function validateStep(step: number, formData: Record<string, any>): Record<string, string> {
  ...
  if (step === 0) {
    ...
    // Treat an untouched field as 0 — the input displays 0 by default, so
    // requiring interaction would reject a value the user can see is valid.
    if (d.dependants === '') {
      errors.dependants = 'Number of dependants is required';
    } else {
      const dep = d.dependants === undefined || d.dependants === null ? 0 : parseInt(d.dependants);
      if (isNaN(dep) || dep < 0) {
...
function PaymentSection({ formData, updateField, applicationId }: { formData: any; updateField: any; applicationId: string | null }) {
  ...
  const downloadConfirmationPdf = () => {
    const ref = submitRef || application.referenceNumber || 'IAAS-2026-XXXXX';
    const personal = formData.personal || {};
    const address = formData.address || {};
    const printWindow = window.open('', '_blank');
    if (!printWindow) {
      alert('Please allow pop-ups for this site to download your confirmation.');
      return;
    }
    ...
    printWindow.document.write(`
      <html>
      <head>
        <title>IAAS Application Confirmation — ${ref}</title>
        <style>
          ...
          .ref { font-size: 20px; font-weight: bold; font-family: monospace; color: #00703c; }
      ...
      <body>
        <div class="header">
          ...
          <div class="subtitle">Initial Application Advice Service — Application Confirmation</div>
        </div>

        <h1>Application Confirmation</h1>
        <p><strong>Reference Number:</strong> <span class="ref">${ref}</span></p>
        <p><strong>Submitted:</strong> ${new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })} at ${new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</p>
        <p><strong>Status:</strong> ${savedLocally ? 'Saved — pending submission' : 'Submitted'}</p>
        <p><strong>Application fee:</strong> £90.00 (SANDBOX — no real payment taken)</p>

        <h2>Applicant Details</h2>
        <table>
          <tr><th>Name</th><td>${[personal.title, personal.firstName, personal.lastName].filter(Boolean).join(' ') || '—'}</td></tr>
          <tr><th>Date of Birth</th><td>${personal.dateOfBirth || '—'}</td></tr>
          <tr><th>National Insurance No.</th><td>${personal.nationalInsuranceNumber || '—'}</td></tr>
          <tr><th>Marital Status</th><td>${personal.maritalStatus || '—'}</td></tr>
          <tr><th>Employment Status</th><td>${personal.employmentStatus || '—'}</td></tr>
          <tr><th>Dependants</th><td>${personal.dependants ?? 0}</td></tr>
          <tr><th>Address</th><td>${[address.line1, address.line2, address.city, address.postcode].filter(Boolean).join(', ') || '—'}</td></tr>
        </table>
        ...
        <p><strong>Keep your reference number.</strong> You will need it to track your application or contact us about it.</p>

        <div class="footer">
          <p><strong>Disclaimer:</strong> This confirmation was generated by a proof-of-concept demonstration system. No real application has been lodged with Accountant in Bankruptcy and no payment has been taken.</p>
          <p>Accountant in Bankruptcy • Scottish Government</p>
          <p>Document ID: CONF-${ref}</p>
        </div>
      </body>
      </html>
    `);
      ...
      <div className="animate-[fadeIn_0.6s_ease-in]">
        ...
        <div className="bg-white dark:bg-gray-800 border border-t-0 border-gray-200 dark:border-gray-700 rounded-b p-6">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            <button onClick={downloadConfirmationPdf} className="bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-gray-100 font-bold py-2 px-4 text-sm rounded hover:bg-gray-300 flex items-center gap-2">
